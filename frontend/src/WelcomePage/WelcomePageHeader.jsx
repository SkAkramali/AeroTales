...
const Nav = function () {
  ...
  const handleLoginStatus = async function () {
    const respone = await fetch('http://localhost:8000/isLoggedIn', {
      method: "GET",
      credentials: "include",
    });
  ...
  return (
    <nav>
      <ul>
        <li>Our story</li>
        <li>Membership</li>
        <li onClick={singIn}>Write</li>
        <li onClick={singIn}>Sign in</li>
        <li><button className="button getStart" onClick={singIn}>Get Start</button></li>
      </ul>
    </nav>
  )
};

const WelcomePageHeader = function () {
  return (
